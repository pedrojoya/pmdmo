...
public class MetadatosActivity extends Activity {
	...
	private void initVistas() {
		...
		Cursor cursor = getContentResolver().query(uriFoto, 
				new String[] {Media.DISPLAY_NAME, Media.DESCRIPTION}, null, null, null);
		if (cursor != null) {
			cursor.moveToFirst();
	...
	public void btnGuardarOnClick(View v) {
		// Creo un ContentValues y lo relleno con los valores de título y descripción.
		...
		// Actualizo en la MediaStore los metadatos de la foto e informo al usuario.
		getContentResolver().update(uriFoto, metadatos, null, null);
		Toast.makeText(this, R.string.metadatos_guardados, Toast.LENGTH_SHORT).show();
