...
public class MainActivity extends Activity {
    ...
    private class AdaptadorAlbumes extends ArrayAdapter<Album> {
		...
		public AdaptadorAlbumes(Context contexto, ArrayList<Album> albumes) {
			...
			this.todos = (ArrayList<Album>) this.albumes.clone();	// Copia del original.
		...
		public Filter getFilter() {
			...
			return new Filter(){
				// Se ejecuta cuando se debe filtrar. Recibe la cadena ya introducida.
				@Override
				protected FilterResults performFiltering(CharSequence constraint) {
					...
					if (constraint != null) {
		                ...
		                sugerencias.clear();
		                // Compruebo uno a uno todos los álbumes por si debemos incluirlo en las sugerencias.
		                ...
		                filterResults.values = sugerencias;			// Valores de las sugerencias.
		                filterResults.count = sugerencias.size();	// Número de sugerencias.
		                return filterResults;
		            } else {
		            	// Si no se ha introducido texto retorno un resultado vacío.
		                return new FilterResults();
		            }		
				}

				// Se encarga de publicar el resultado seleccionado en el widget.
				@Override
				protected void publishResults(CharSequence constraint, FilterResults results) {
					...
					if (results != null && results.count > 0) {
						...
						// Lleno el array de álbumes con la lista de álbumes filtrados.
						// OJO! Se debe usar clear() y add() del adaptador, para modificar
						// el array de datos que maneja el adaptador.
		                clear();
		                for (Album album : albumesFiltrados) {
		                    add(album);
		                }
		                // Notifico que se han producido cambios en el conjunto de datos
		                // para que la vistas se repinten. (¿obligatorio en este caso?)
		                notifyDataSetChanged();
		            }
					
				}

				// Retorna la cadena que debe escribirse en el widget. Recibe el objeto seleccionado
				@Override
		...
		// Obtiene la vista-fila que se va a mostrar para un elemento dada su posición.
		@Override
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// OJO! usar getItem del Adaptador para obtener el album correspondiente
    ...
    private void getVistas() {
		...
		albumes.add(new Album(R.drawable.cantecito, "Echate un cantecito", "1992"));
